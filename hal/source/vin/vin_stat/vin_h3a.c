...
static int isp_stat_bufs_alloc(struct isp_stat *stat, u32 size, u32 count)
{
	...
	//isp_addr_size = ISP_SAVE_DRAM_SIZE + ISP_SAVE_LOAD_DRAM_SIZE + 3 * ISP_LOAD_DRAM_SIZE;
	...
	for (i = 1; i < stat->buf_cnt; i++) {
		struct ispstat_buffer *buf = &stat->buf[i];
		//buf->phy_addr = MEMRESERVE + 2 * isp_addr_size + (isp->id * (stat->buf_cnt - 1) * size) + ((i - 1) * size);
		buf->phy_addr = rt_memheap_alloc_align(&isp_mempool, stat->buf_size, 0x100);
	...
	isp->isp_stat_buf.size = ISP_STAT_TOTAL_SIZE + ISP_SAVE_LOAD_STATISTIC_SIZE;
	//isp->isp_stat_buf.phy_addr = MEMRESERVE + (2 * isp_addr_size) + (2 * (stat->buf_cnt - 1) * size) + isp->id * isp->isp_stat_buf.size;
	isp->isp_stat_buf.phy_addr = rt_memheap_alloc_align(&isp_mempool, isp->isp_stat_buf.size, 0x100);
