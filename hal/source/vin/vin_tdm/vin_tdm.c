		...
		if (tdm_buf_num) {
			...
			if (size) {
				...
				for (i = 0; i < tdm_buf_num; i++) {
					tdm_rx->buf[i].dma_addr = rt_memheap_alloc_align(&isp_mempool, size, 0x100);
...
int tdm_probe(unsigned int id)
{
	...
	tdm->base = vin_tdm_base[id];
	if (!tdm->base) {
		tdm->is_empty = 1;
	} else {
		tdm->is_empty = 0;
		/*get irq resource */
		tdm->irq = vin_tdm_irq[id];
		if (tdm->irq <= 0) {
...
{
	struct tdm_dev *tdm = glb_tdm[id];
	int i;

	if (!tdm_probe_flag[id])
		return 0;
	else
		tdm_probe_flag[id] = false;
