...
static int isp_config_sensor_info(struct isp_dev *isp)
{
	...
	sensor_format = global_sensors[sensor_id].sensor_core->sensor_formats;
...
static int isp_alloc_reg_tbl(struct isp_dev *isp)
{
	...
	//unsigned int isp_vipp_addr_size = 2 * (ISP_SAVE_DRAM_SIZE + ISP_SAVE_LOAD_DRAM_SIZE + 3 * ISP_LOAD_DRAM_SIZE) + ((VIPP_REG_SIZE * 3) * 16);
	//unsigned int stat_add_size = (((3 - 1) * ISP_STAT_TOTAL_SIZE) + (ISP_STAT_TOTAL_SIZE + ISP_SAVE_LOAD_STATISTIC_SIZE)) * 2;

	isp->server_stat.size = ISP_LOAD_DRAM_SIZE;
	/* REG LOAD*/
	//isp->server_stat.phy_addr = MEMRESERVE + isp_vipp_addr_size + stat_add_size + isp->id * isp->server_stat.size;
	isp->server_stat.phy_addr = rt_memheap_alloc_align(&isp_mempool, isp->server_stat.size, 0x100);
...
void vin_server_update(struct isp_dev *isp, struct isp_part_para isp_part)
{
	...
	//ae_result_t *ae_result = NULL;
	struct isp_lib_context *isp_gen;
	__maybe_unused struct isp_param_config *param;
	...
	if (!isp_server_save_reg(isp->id, ir)) {
#if 1
		/*now get the right tmp and lum, so can calculate correct except that need by statitic*/
		param = &isp_gen->isp_ini_cfg;
		isp_gen->module_cfg.module_enable_flag &= ~ISP_FEATURES_PLTM;
		isp_gen->module_cfg.module_enable_flag &= ~ISP_FEATURES_RGB_DRC;
		isp_hardware_update(&isp_gen->module_cfg);
		if (param->isp_test_settings.pltm_en) {
			isp_gen->module_cfg.module_enable_flag |= ISP_FEATURES_PLTM;
			*((int *)isp_gen->load_reg_base + 0x1a0/4) = *((int *)isp_gen->load_reg_base + 0x1a0/4) | PLTM_EN;
		}
		if (param->isp_test_settings.drc_en) {
			isp_gen->module_cfg.module_enable_flag |= ISP_FEATURES_RGB_DRC;
			*((int *)isp_gen->load_reg_base + 0x1a0/4) = *((int *)isp_gen->load_reg_base + 0x1a0/4) | RGB_DRC_EN;
		}
#else
		/* use high frame rate WB gain*/
		*((int *)isp_gen->load_reg_base + 0x370/4) = isp_gen->module_cfg.wb_gain_cfg.wb_gain.r_gain +
								(isp_gen->module_cfg.wb_gain_cfg.wb_gain.gr_gain<<16);
		*((int *)isp_gen->load_reg_base + 0x374/4) = isp_gen->module_cfg.wb_gain_cfg.wb_gain.gb_gain +
								(isp_gen->module_cfg.wb_gain_cfg.wb_gain.b_gain<<16);

		/* use high frame rate dgain*/
		*((int *)isp_gen->load_reg_base + 0x360/4) = isp_gen->module_cfg.gain_offset_cfg.gain.r_gain +
								(isp_gen->module_cfg.gain_offset_cfg.gain.gr_gain<<16);
		*((int *)isp_gen->load_reg_base + 0x364/4) = isp_gen->module_cfg.gain_offset_cfg.gain.gb_gain +
								(isp_gen->module_cfg.gain_offset_cfg.gain.b_gain<<16);
		*((int *)isp_gen->load_reg_base + 0x1f4/4) = *((int *)isp_gen->load_reg_base + 0x360/4);
		*((int *)isp_gen->load_reg_base + 0x1f8/4) = *((int *)isp_gen->load_reg_base + 0x364/4);
		*((int *)isp_gen->load_reg_base + 0x10c/4) = *((int *)isp_gen->load_reg_base + 0x360/4);
		*((int *)isp_gen->load_reg_base + 0x110/4) = *((int *)isp_gen->load_reg_base + 0x364/4);

		int i;
		/*saturation*/
		if (!ir) {
			for (i = 0; i < 128; i++)
				// 0xfff2fff2:lim_idx = 120, 0x00010001:lim_idx = 118
				*((int *)isp_gen->module_cfg.saturation_table + i) = 0x00010001; //lim_idx = 118
		}
#endif
#if defined CONFIG_D3D
		if ((isp->load_shadow[0x320 + 0x0]) & (1<<0)) {
		...
		//if (ae_result->sensor_set.ev_set_curr.ev_idx > ae_result->sensor_set.ev_idx_max*65/100) {
			memcpy(isp_gen->load_reg_base + 0x2a0, &isp_bdnf_reg[0], 4 << 2);
			memcpy(isp_gen->load_reg_base + 0x9b8, &isp_bdnf_reg[4], (17*3) << 2);
		//}

		/* gamma make dark darker*/
		//*((int *)isp_gen->load_reg_base + 0x1a0/4) = *((int *)isp_gen->load_reg_base + 0x1a0/4) & (~(1<<9));

		memcpy(&isp->load_shadow[0], isp_gen->load_reg_base, ISP_LOAD_DRAM_SIZE - ISP_LOAD_DEBUG_DRAM_SIZE);
		isp->load_flag = 1;
	} else {
		memcpy(isp_gen->load_reg_base, &isp_default_reg[0], ISP_LOAD_REG_SIZE);
		isp_set_load_reg(isp);
...
int vin_isp_ae_done(struct isp_dev *isp, int drap_frame_en)
{
	...
	if (!drap_frame_en ||
		...
		if (drap_frame_en) {
			vin_print("isp%d:high frame rate convergence! isp_count:%d, ae_conut:%d, awb_count:%d\n",
				isp->id, isp->isp_frame_number, isp_gen->ae_frame_cnt, isp_gen->awb_frame_cnt);
		}
		return 0;
	}
#else
	if (!drap_frame_en ||
			((ae_result->ae_status == AE_STATUS_DONE || isp_gen->ae_frame_cnt >= 0))) {
		if (drap_frame_en) {
