...
void __isp_gtm_run(struct isp_lib_context *isp_gen)
{
	...
	if (isp_gen->isp_ini_cfg.isp_tunning_settings.gtm_type == 4) {
		/*ldci_frame_buf_addr = ldci_video_get_frame(LDCI_VIDEO_CHN, &buffer);
		if (ldci_frame_buf_addr == NULL)
			return;
		isp_gen->gtm_entity_ctx.gtm_param->ldci_frame_buf_addr = ldci_frame_buf_addr;
		isp_gtm_cxt->ops->isp_gtm_run(isp_gtm_cxt->gtm_entity,
				&isp_gtm_cxt->gtm_stats,
				&isp_gtm_cxt->gtm_result);
		ldci_video_free_frame(LDCI_VIDEO_CHN, buffer);*/
...
void __get_sensor_temp_param(struct isp_lib_context *isp_gen)
{
	...
	if (isp_gen->temp_info.enable) {
		...
		ISP_LIB_LOG(ISP_LOG_ISO, "sensor temperature=%d.  temperature_param = %d %d %d %d %d %d %d %d %d %d %d %d\n", isp_gen->sensor_info.temperature,
					...
					isp_gen->temp_info.temperature_param[TEMP_COMP_BLC_R], isp_gen->temp_info.temperature_param[TEMP_COMP_BLC_GR],
					isp_gen->temp_info.temperature_param[TEMP_COMP_BLC_GB], isp_gen->temp_info.temperature_param[TEMP_COMP_BLC_B],
		...
		isp_gen->temp_info.temperature_param[TEMP_COMP_BLC_GR] = 0;
		isp_gen->temp_info.temperature_param[TEMP_COMP_BLC_GB] = 0;
