...
static struct isp_pix_fmt *__isp_try_format(unsigned int id, unsigned int vinc_id)
{
	...
	sensor_format = global_sensors[mipi_sel].sensor_core->sensor_formats;
...
#if 0
	switch (sensor_formats->colorspace) {
...
int sunxi_isp_subdev_s_stream(unsigned int id, unsigned int vinc_id, int enable)
{
	...
	if (enable) {
		...
		hal_workqueue_cancel_work(isp->isp_server_run_task, &isp->start_isp_server_work);
...
static int isp_resource_alloc(unsigned int id)
{
	...
	//isp->isp_stat.phy_addr = (unsigned long)MEMRESERVE + id * isp->isp_stat.size;
	isp->isp_stat.phy_addr = rt_memheap_alloc_align(&isp_mempool, isp->isp_stat.size, 0x100);
...
int isp_probe(unsigned int id)
{
	...
	isp->isp_server_run_task = hal_workqueue_create("isp", 16*1024, 15);
