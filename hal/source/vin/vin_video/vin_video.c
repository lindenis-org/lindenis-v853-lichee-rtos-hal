...
{
	//unsigned int isp_vipp_addr_size, stat_add_size, server_load_size;
	//isp_vipp_addr_size = (ISP_SAVE_DRAM_SIZE + ISP_SAVE_LOAD_DRAM_SIZE + 3 * ISP_LOAD_DRAM_SIZE) * 2 + ((VIPP_REG_SIZE * 3) * 16);
	//stat_add_size = (((3 - 1) * ISP_STAT_TOTAL_SIZE) + (ISP_STAT_TOTAL_SIZE + ISP_SAVE_LOAD_STATISTIC_SIZE)) * 2;
	//server_load_size = ISP_LOAD_DRAM_SIZE * 2;
	//isp_vipp_addr_size = roundup(isp_vipp_addr_size, 0x100);
	//vin_print("isp_vipp_addr size is 0x%x\n", isp_vipp_addr_size);
	unsigned int size;
	unsigned int i;
	...
	for (i = 0; i < BUF_NUM; i++) {
		...
		buff[i].phy_addr = (unsigned int *)(MEMRESERVE + isp_vipp_addr_size + i*sensor_formats->o_width * sensor_formats->o_height * 3/2);
		if (i == 0)
			vin_log(VIN_LOG_MD, "%s: buffer[%d] phy_addr is 0x%x\n", __func__, i, (unsigned int)buff[i].phy_addr);
	}
#else
	//buff[0].phy_addr = (unsigned long)(MEMRESERVE + isp_vipp_addr_size + stat_add_size + server_load_size);
	//buff[1].phy_addr = (unsigned long)(buff[0].phy_addr + global_video[id].o_width * global_video[id].o_height * 3 / 2);
	//buff[2].phy_addr = (unsigned long)(buff[1].phy_addr + global_video[id].o_width * global_video[id].o_height * 3 / 2);
	for (i = 0; i < 1; i++) {
		global_vinc[id]->buff[i].phy_addr = rt_memheap_alloc_align(&isp_mempool, size, 0x100);
		if (global_vinc[id]->buff[i].phy_addr == NULL) {
			vin_err("%s:video%d:alloc bk buffer%d error\n", __func__, id, i);
			return -1;
		}
	}
	vin_log(VIN_LOG_MD, "video%d: buffer[0] phy_addr is 0x%x\n", id, (unsigned int)global_vinc[id]->buff[0].phy_addr);
#endif
#else
	/* buff.phy_addr = (unsigned int *)malloc_align(sensor_formats->o_width * sensor_formats->o_height * 3/2, 0x100); */
	//buff->phy_addr = (unsigned long)(MEMRESERVE + isp_vipp_addr_size + stat_add_size + server_load_size);
	global_vinc[id]->buff[0].phy_addr = rt_memheap_alloc_align(&isp_mempool, size, 0x100);
...
int vin_subdev_s_stream(unsigned int id, int enable)
{
	...
	sensor_format = global_sensors[id].sensor_core->sensor_formats;
