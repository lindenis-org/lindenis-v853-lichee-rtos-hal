...
int sunxi_scaler_subdev_s_stream(unsigned int id, int enable)
{
	...
	sensor_format = global_sensors[scaler->id].sensor_core->sensor_formats;
...
{
	struct scaler_dev *scaler = global_scaler[id];
	//unsigned int isp_addr_size;
	//unsigned int stat_add_size;

	//isp_addr_size = (ISP_SAVE_DRAM_SIZE + ISP_SAVE_LOAD_DRAM_SIZE + 3 * ISP_LOAD_DRAM_SIZE) * 2;
	//stat_add_size = (((3 - 1) * ISP_STAT_TOTAL_SIZE) + (ISP_STAT_TOTAL_SIZE + ISP_SAVE_LOAD_STATISTIC_SIZE)) * 2;
	//isp_addr_size = roundup(isp_addr_size, 0x100);
	//vin_print("isp_addr size is 0x%x\n", isp_addr_size);

	scaler->vipp_reg.size = 3 * VIPP_REG_SIZE;
	//scaler->vipp_reg.phy_addr = (unsigned long)(MEMRESERVE + id * scaler->vipp_reg.size + isp_addr_size + stat_add_size);
	scaler->vipp_reg.phy_addr = rt_memheap_alloc_align(&isp_mempool, scaler->vipp_reg.size, 0x100);
